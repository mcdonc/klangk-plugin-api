...
///
/// Lives in the plugin API (not the host frontend) so that compiled-in
/// feature packages can paint with the same design tokens as the host shell
/// without importing the host package (which would be a package cycle).
/// The frontend re-exports this so existing `KColors.*` usages there are
/// unchanged (#1976).
class KColors {
  KColors._();
